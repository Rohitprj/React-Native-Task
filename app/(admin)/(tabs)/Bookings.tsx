...
//   const [stores, setStores] = useState<Store[]>([]);
//   const [packages, setPackages] = useState<Package[]>([]);
//   const [dataLoading, setDataLoading] = useState<boolean>(true); // Indicates if dropdown data is loading
//   const [dataError, setDataError] = useState<string | null>(null);
...
//   // --- NEW STATE TO TRACK IF INITIAL PARAM FILTER IS APPLIED ---
//   const [initialFilterAppliedFromParams, setInitialFilterAppliedFromParams] =
//     useState<boolean>(false);
...
//   // --- GET PARAMS FROM URL ---
//   const params = useLocalSearchParams();
//   // Assuming the param key is 'storeId' and it comes as a string, convert to number
//   const storeIdFromParams = params.storeId
//     ? parseInt(params.storeId as string, 10)
//     : undefined;
//   console.log("Store Id", storeIdFromParams);
//   // --- NEW CENTRALIZED BOOKING LOADER FOR FILTERS ---
//   const loadBookingsFiltered = async (
//     filterType: "all" | "store" | "status" | "paid" | "customerType" | "date",
//     value?: any // value can be number, string, or Date
...
//   // Initial data load for dropdowns (runs once on mount)
//   useEffect(() => {
//     loadDropdownData();
//   }, []);

//   // Effect to handle initial booking load based on URL params or default to all
//   // This effect should run only ONCE after initial render and dropdown data is potentially loaded.
//   // It will determine the initial state of the bookings list.
//   useEffect(() => {
//     // Only proceed if dropdown data has loaded AND the initial filter hasn't been applied yet
//     if (dataLoading || initialFilterAppliedFromParams) {
//       return;
//     }

//     if (
//       storeIdFromParams &&
//       !isNaN(storeIdFromParams) &&
//       storeIdFromParams > 0
//     ) {
//       // If a valid storeId is in params, apply that filter
//       setSelectedFilterStoreId(storeIdFromParams);
//       setSelectedFilterStatus("all");
//       setSelectedFilterPaidStatus("all");
//       setSelectedFilterCustomerType("all");
//       setSelectedFilterDate(null);
//       loadBookingsFiltered("store", storeIdFromParams);
//     } else {
//       // No valid storeId in params, load all bookings (default behavior)
//       setSelectedFilterStoreId(0); // Ensure "All Stores" is selected visually
//       loadBookingsFiltered("all");
//     }
//     setInitialFilterAppliedFromParams(true); // Mark initial filter as applied
//   }, [storeIdFromParams, dataLoading, initialFilterAppliedFromParams]); // Dependencies for this initial load logic
...
//           style={styles.retryButton}
//           onPress={() => {
//             // Re-trigger both data loads
//             loadBookingsFiltered("all"); // This will be superseded by the param effect if present
//             loadDropdownData();
//           }}
//         >
//           <Text style={styles.buttonText}>Retry</Text>
