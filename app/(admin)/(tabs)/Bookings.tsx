...
//   /* <View>
//         <Picker
//           style={{ ...styles.filterPickerStyle, backgroundColor: "red" }}
//           itemStyle={styles.filterPickerItemStyle}
//         >
//           <Picker.Item label="All Stores" value={0} />
//           {[{ id: 1, name: "rohit" }].map((store) => (
//             <Picker.Item key={store.id} label={store.name} value={store.id} />
//           ))}
//         </Picker>
//       </View> */
...
//   // --- GET PARAMS FROM URL ---
//   const params = useLocalSearchParams();
//   const storeNameFromParams = params.storeName as string | undefined;
...
//   // Initial data load on component mount for all bookings and dropdowns
...
//   // --- NEW: Effect to handle storeName from params and apply filter ---
//   useEffect(() => {
//     // Only proceed if storeNameFromParams exists AND stores data is loaded
//     if (storeNameFromParams && stores.length > 0) {
//       const targetStore = stores.find(
//         (store) => store.name === storeNameFromParams
//       );
//       if (targetStore) {
//         // If the store is found, set the filter and load data
//         // Only update if the filter is not already set to this store,
//         // to prevent unnecessary re-renders and potential loops.
//         if (selectedFilterStoreId !== targetStore.id) {
//           setSelectedFilterStoreId(targetStore.id);
//           // Reset other filters when a store-specific filter is applied
//           setSelectedFilterStatus("all");
//           setSelectedFilterPaidStatus("all");
//           setSelectedFilterCustomerType("all");
//           setSelectedFilterDate(null);
//           loadBookingsFiltered("store", targetStore.id);
//         }
//       } else {
//         // If storeName from params doesn't match any store, log it
//         console.warn(`Store "${storeNameFromParams}" not found.`);
//         // Revert to "All Stores" if the parameter was invalid, but only if not already "All Stores"
//         if (selectedFilterStoreId !== 0) {
//           setSelectedFilterStoreId(0);
//           loadBookingsFiltered("all"); // Load all bookings
//         }
//       }
//     }
//     // IMPORTANT: Removed the 'else if (!storeNameFromParams && !loading)' block.
//     // The initial useEffect already handles the default state where no param is present.
//   }, [storeNameFromParams, stores, selectedFilterStoreId]); // Dependencies: storeNameFromParams, stores (for data), and selectedFilterStoreId (to avoid re-triggering if already set)
