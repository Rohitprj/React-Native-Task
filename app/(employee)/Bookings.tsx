...
} from "@/utils/bookingApiEmp"; // Using emp-specific APIs
import {
...
import axiosInstance from '@/utils/axiosInstance'; // Using your provided axiosInstance
import {
  fetchCustomersApi,
  fetchPackagesApi,
  fetchStoresApi,
...
// Custom Message Modal Component (re-used from previous admin version)
...
const BookingsScreen = () => {
  ...
  const [filteredBookings, setFilteredBookings] = useState<DisplayBookingItem[]>([]); // State for filtered bookings
  ...
  const loadBookingsFiltered = async (
    ...
    try {
      ...
      const mappedData: DisplayBookingItem[] = apiData.map((booking) => ({
        ...
        status: booking.paid ? "PAID" : "UNPAID", // Display payment status for the table
        paid: booking.paid,
  ...
  const getBookingDetails = async (bookingId: number) => {
    try {
      const response = await axiosInstance.get(
        `/emp/booking/details/${bookingId}` // Using emp API for details
      );
      if (response.data.valid) {
        ...
        const mappedBooking: DisplayBookingItem = {
          ...
          phone: booking.customer?.phone || "N/A",
          bookingsTypes: booking.bookingType || "N/A",
          status: booking.paid ? "PAID" : "UNPAID", // Map directly to PAID/UNPAID based on 'paid' boolean
  ...
  // Function to handle overs played submission (Employee version)
  const handleSubmitOversPlayed = async () => {
    ...
    try {
      const response = await axiosInstance.put(
        `/emp/booking/${selectedBooking.id}`, // Using emp API for updating booking
        { playedOvers: overs }
      );
      if (response.data.valid) {
        ...
        getBookingDetails(selectedBooking.id); // Refresh details in modal
        loadBookingsFiltered("all"); // Refresh main list
      } else {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {/* Removed Store filter as per employee version */}
        ...
        {/* Removed Paid Status filter as per employee version */}
        {/* <View style={styles.filterPickerContainer}>
          <Picker
        ...
        {/* Removed Customer Type filter as per employee version */}
        {/* <View style={styles.filterPickerContainer}>
          <Picker
      ...
      <View style={styles.searchRow}>
        ...
        />
        {/* Removed Existing button as per employee version */}
        {/* <TouchableOpacity
          ...
          <View style={[styles.formContainer, { alignItems: "stretch" }]}>
            ...
            {selectedBooking && (
              <>
                ...
                {/* Removed Mark Payment as Done Button for employee version */}
                {/* {!selectedBooking.paid && (
                  <TouchableOpacity
                    style={{
      ...
      {/* New Direct Booking Modal (re-used from admin version as employee can also create direct bookings) */}
      <Modal
        animationType="slide"
        transparent={true}
      ...
      {/* Existing Customer Booking Modal (re-used from admin version) */}
      <Modal
        animationType="slide"
        transparent={true}
...
const styles = StyleSheet.create({
  ...
  typeSelectorContainer: {
    flexDirection: "row",
    backgroundColor: Colors.STB.background, // Adjusted for white background
    borderRadius: 8,
    borderWidth: 1,
    borderColor: Colors.STB.buttons, // Adjusted for white background
    overflow: "hidden",
    marginBottom: 15,
  },
  typeSelectorButton: {
  ...
  typeSelectorText: {
    color: Colors.STB.text, // Adjusted for white background
    fontWeight: "600",
    fontSize: 15,
  },
  ...
  pickerItemStyle: {
    color: "#000", // iOS specific - Adjusted for white background
    fontSize: 16,
  },
  filterRow: {
    flexDirection: "row",
